<template>
  <div>
    <div v-clickoutside="hideReplyBtn" @click="inputFocus" class="my-reply">
      <el-avatar style="display: flex" class="header-img" :size="40" :src="myHeader"></el-avatar>
      <p style="display: flex">{{myName}}</p>
...
export default {
  ...
  mounted() {
    ...
    this.id=localStorage.getItem("chenletoken")
    this.axios({
      url: "course/common/getcommon/",
      params:{
    ...
    this.axios({
      ...
      // console.log(res.data.page)
      this.myId = res.data.user.id
      this.myName=res.data.user.username
    })
  },
  ...
  methods: {
    ...
    sendComment(){
      if(!this.replyComment){
        ...
        console.log()
        this.axios({
          url: "course/common/save/",
          data:{
            userId:this.myId,
            courseId:this.courseId,
            information:this.replyComment,
            time:new Date(),
            zan:Math.floor(Math.random() * 50 + 1),
          },
          method: "post"
        }).then((res) => {
